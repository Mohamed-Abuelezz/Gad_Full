                            ...
                            <table id="datatablesSimple">
                                ...
                                <tbody>
                                 ...
                                 <tr>
                                    ...
                                    <td>
                                        <a href="{{ asset('storage/images/users_images/'.$profile->user->image) }}" target="_blank" class="preview" style="z-index: 9999" title=" Sherif Nabil Abo.elezz ">
                                            <img class="profile-image "
                                            src="{{ asset('storage/images/users_images/'.$profile->user->image) }}" width="100%" height="100"/>
                                          </a>
                                    </td>
                                    <td>{{$profile->display_name}}</td>
                                    <td>{{$profile->profiles_type->name_ar}}</td>
                                    <td>{{$profile->phone}}</td>
                                    <td>lat :- {{$profile->lat}} , lng:- {{$profile->lng}}</td>
                                    <td>{{$profile->bio}}</td>

                                    <td>
                                     <a class="btn btn-primary" href="{{url('admin/addorEditProfile/'.$profile->id)}}" role="button">تعديل</a>
                                      <a class="btn btn-danger" href="{{url('admin/deleteProfile/'.$profile->id)}}" role="button">حذف</a>
                                    </td>
                                </tr>
                                 @endforeach
